# 🎁 FIAP ON + GitHub Student Developer Pack: Garanta Mais de $100 MIL Reais em Benefícios Exclusivos

🧭 Este repositório é um **pacote organizado**: contexto do GitHub Education, passo a passo de solicitação, sugestões do que ativar no começo, **catálogo** para consulta quando precisar e **extras** (links, canais, roadmap + bookmarklet de leitura em voz).

> ⭐ Ei! Ajude o coleguinha, de uma estrela / like
> 📅 **Última atualização:** 26/03/2026

---

## 🧐 O que tem neste repositório

| 🎯 | 📄 Documento | 💬 Em poucas palavras |
|---|-----------|-------------------|
| 📌 | [**1. Guia principal**](docs/01-github-student-pack.md) | O que é o Github Pack, requisitos, solicitação e o que faz sentido ativar primeiro |
| 📂 | [**2. Catálogo**](docs/02-pack-catalog.md) | Explore com mais atenção o catálogo de benefícios completo |
| 🎁 | [**3. Bônus**](docs/03-bonus-resources.md) | Acesso grátis a biblioteca de livros caros, canais YouTube, Links e Script Exclusivo |

> 📑 **Índice da pasta** `docs/`: [ver `docs/README.md`](docs/README.md).

## 🖐️ Antes de ativar um monte de coisa

- Leia o **guia principal** antes de sair clicando em tudo.  
- Ative **só** o que conversa com o que você estuda ou está construindo **agora**.  
- Valores em R$ citados nos textos são **estimativas ilustrativas** de economia potencial — dependem do uso e mudam com o tempo.

---

## ❤️ Contribuir

Se quiser apoiar financeiramente o trabalho por trás deste e de outros projetos open source, existe a página no **GitHub Sponsors**: [github.com/sponsors/wilamis-brasil](https://github.com/sponsors/wilamis-brasil).

Issues e PRs são bem-vindos para correções e melhorias. Veja [**CONTRIBUTING.md**](CONTRIBUTING.md).

---

## ⚖️ Licença

Conteúdo sob [**CC BY 4.0**](LICENSE) (Creative Commons **Atribuição** 4.0) · © 2026 Wilamis Brasil.

**É obrigatório dar créditos** ao reutilizar ou republicar este material (nome do autor + link ao repositório ou à licença). Detalhes no arquivo [LICENSE](LICENSE).

⭐ Se o material te ajudou, uma estrela ajuda outras pessoas a encontrarem o guia.
